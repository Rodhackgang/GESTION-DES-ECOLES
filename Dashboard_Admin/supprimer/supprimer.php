...
if (isset($_POST['id'])) {
    ...
    try {
        ...
        header("Location: ../../Dashboard_Admin/gestion_eleves.php?success=deleted");
        exit();
    } catch (Exception $e) {
        // En cas d'échec, annuler la transaction et rediriger avec un message d'erreur
        $conn->rollback();
        header("Location: ../../Dashboard_Admin/gestion_eleves.php?error=" . urlencode($e->getMessage()));
        exit();
    }
} else {
    // Si l'identifiant de l'élève n'a pas été envoyé, rediriger avec un message d'erreur
    header("Location: ../../Dashboard_Admin/gestion_eleves.php?error=no_id");
    exit();
}
