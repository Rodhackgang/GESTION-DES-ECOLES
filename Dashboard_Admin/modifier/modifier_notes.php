...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($required_fields as $field) {
        if (!isset($_POST[$field])) {
            header("Location: ../../Dashboard_Admin/gestion_note.php?error=" . urlencode("Toutes les données nécessaires n'ont pas été fournies"));
            exit();
        }
    }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../../Dashboard_Admin/gestion_note.php?success=" . urlencode("Les données de la note ont été modifiées avec succès"));
        exit();
    } else {
        // Redirection avec un message d'erreur
        header("Location: ../../Dashboard_Admin/gestion_note.php?error=" . urlencode("Erreur lors de la modification des données de la note : " . $conn->error));
        exit();
    }
    ...
    header("Location: ../../Dashboard_Admin/gestion_note.php?error=" . urlencode("Requête non autorisée"));
    exit();
}
?>
