...
$moyenneGeneraleEleve = $moyenneGenerale;

// Récupérer les moyennes générales de tous les élèves
$query = "SELECT eleves.id, AVG(notes.note) AS moyenne_generale
          FROM eleves
...
if ($result->num_rows > 0) {
    // Stocker les moyennes générales des élèves dans un tableau
    $moyennesGeneralesEleves = [];
    while ($row = $result->fetch_assoc()) {
        $moyennesGeneralesEleves[$row['id']] = $row['moyenne_generale'];
    }

    // Trier les moyennes générales par ordre décroissant
    arsort($moyennesGeneralesEleves);

    // Trouver le rang de l'élève
    $rangEleve = array_search($bulletin['eleve_id'], array_keys($moyennesGeneralesEleves)) + 1;

    // Afficher le rang de l'élève
    echo "<div class='notes'>";
    echo "<h4>Rang de l'élève: $rangEleve</h4>";
    echo "</div>";
} else {
