...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['id'], $_POST['nom'], $_POST['description'], $_POST['enseignant'], $_POST['salle_classe'])) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            header("Location: ../../../Dashboard_Staff/Proviseur/gestion_matiere.php?success=" . urlencode("Les données de la matière ont été modifiées avec succès"));
            exit();
        } else {
            // Erreur lors de la modification
            header("Location: ../../../Dashboard_Staff/Proviseur/gestion_matiere.php?error=" . urlencode("Erreur lors de la modification des données de la matière : " . $conn->error));
            exit();
        }
    } else {
        // Données manquantes dans la requête POST
        header("Location: ../../../Dashboard_Staff/Proviseur/gestion_matiere.php?error=" . urlencode("Toutes les données nécessaires n'ont pas été fournies"));
        exit();
    }
} else {
    // Requête non autorisée
    header("Location: ../../../Dashboard_Staff/Proviseur/gestion_matiere.php?error=" . urlencode("Requête non autorisée"));
    exit();
}
?>
