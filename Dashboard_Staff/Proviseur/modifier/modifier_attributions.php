    ...
    if (isset($_POST['id']) && isset($_POST['cours']) && isset($_POST['salle']) && isset($_POST['date']) && isset($_POST['heure_debut']) && isset($_POST['heure_fin'])) {
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: ../gestion_attribution.php?success=" . urlencode("Les données de l'attribution ont été modifiées avec succès"));
                exit();
            } else {
                // Erreur lors de la modification
                header("Location: ../gestion_attribution.php?error=" . urlencode("Erreur lors de la modification des données de l'attribution : " . $stmt->error));
                exit();
            }
            $stmt->close();
        } else {
            // Erreur lors de la préparation de la requête
            header("Location: ../gestion_attribution.php?error=" . urlencode("Erreur lors de la préparation de la requête : " . $conn->error));
            exit();
        }
        ...
        header("Location: ../gestion_attribution.php?error=" . urlencode("Toutes les données nécessaires n'ont pas été fournies"));
        exit();
    }
} else {
    // Requête non autorisée
    header("Location: ../gestion_attribution.php?error=" . urlencode("Requête non autorisée"));
    exit();
}
?>
