...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['id']) && isset($_POST['nom']) && isset($_POST['capacite']) && isset($_POST['prof_principal'])) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            header("Location: ../../../Dashboard_Staff/Enseigants/gestion_salle.php?success=" . urlencode("Les données de la salle de classe ont été modifiées avec succès"));
            exit();
        } else {
            // Erreur lors de la modification
            header("Location: ../../../Dashboard_Staff/Enseigants/gestion_salle.php?error=" . urlencode("Erreur lors de la modification des données de la salle de classe : " . $conn->error));
            exit();
        }
        ...
        header("Location: ../../../Dashboard_Staff/Enseigants/gestion_salle.php?error=" . urlencode("Toutes les données nécessaires n'ont pas été fournies"));
        exit();
    }
} else {
    // Requête non autorisée
    header("Location: ../../../Dashboard_Staff/Enseigants/gestion_salle.php?error=" . urlencode("Requête non autorisée"));
    exit();
}
?>
